<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.title" :placeholder="$t('equipment.equipment')" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
    ...
    >
      ...
      <el-table-column :label="$t('equipment.equipment')">
        <template slot-scope="{row}">
          <span class="link-type" @click="handleUpdate(row)">{{ row.equipment }}</span>
        </template>
      </el-table-column>

      <el-table-column :label="$t('equipment.quantity')">
        <template slot-scope="{row}">
          <span class="link-type" @click="handleUpdate(row)">{{ row.quantity }}</span>
        </template>
      </el-table-column>

      <el-table-column :label="$t('equipment.rate')">
        <template slot-scope="{row}">
          <span class="link-type" @click="handleUpdate(row)">{{ row.rate }}</span>
        </template>
      </el-table-column>

      <el-table-column :label="$t('equipment.amount')">
        <template slot-scope="{row}">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="170px" style="width: auto; margin-left:50px;">
        ...
        <!-- <el-form-item :label="$t('equipment.date')" prop="timestamp">
          <el-date-picker v-model="temp.timestamp" type="datetime" placeholder="Please pick a date" />
        </el-form-item> -->

        <el-form-item :label="$t('equipment.equipment')" prop="equipment">
          <el-input v-model="temp.equipment" />
        </el-form-item>

        <el-form-item :label="$t('equipment.quantity')" prop="quantity">
          <el-input v-model="temp.quantity" />
        </el-form-item>

        <el-form-item :label="$t('equipment.rate')" prop="rate">
  ...
  methods: {
    async getList(selected_cell, selected_spreadsheet, selected_date) {
      try {
        ...
        const { data } = await fetchEquipment(this.listQuery);
