<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column :label="$t('materialUsedTable.description')" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.description }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('materialUsedTable.unit')">
        <template slot-scope="{row}">
          <span class="link-type" @click="handleUpdate(row)">{{ row.unit }}</span>
        </template>
      </el-table-column>

      <el-table-column :label="$t('materialUsedTable.quantity')">
        <template slot-scope="{row}">
          <span class="link-type" @click="handleUpdate(row)">{{ row.quantity }}</span>
        </template>
      </el-table-column>

      <el-table-column :label="$t('materialUsedTable.price')">
        <template slot-scope="{row}">
          <span class="link-type" @click="handleUpdate(row)">{{ row.price }}</span>
        </template>
      </el-table-column>

      <el-table-column :label="$t('materialUsedTable.amount')">
        <template slot-scope="{row}">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="170px" style="width: auto; margin-left:50px;">
        ...
        <el-form-item :label="$t('materialUsedTable.description')">
        ...
        <el-form-item :label="$t('materialUsedTable.quantity')" prop="quantity">
          <el-input v-model="temp.quantity" />
        </el-form-item>

        <el-form-item :label="$t('materialUsedTable.price')" prop="price">
...
import { fetchMaterialsList, createMaterial, updateMaterial, deleteMaterialsUsed } from '@/api/bill-of-quantities';
  ...
  methods: {
    async getList(selected_cell, selected_spreadsheet, selected_date) {
      try {
        ...
        const { data } = await fetchMaterialsList(this.listQuery);
    ...
    handleModifyStatus(row, status) {
      const vm = this;
      deleteMaterialsUsed(row).then((response) => {
        vm.$emit('refresh');
        this.$notify({
          title: 'Deleting',
    ...
    createData() {
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          this.temp.amount = (this.temp.price * this.temp.quantity);
